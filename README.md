...
## Purpose
The primary goal of this project is to digitize the sales process for a local electronics store. Currently, manual record-keeping is prone to errors. This Python-based application solves that problem by automating stock tracking and invoice generation.

Technically, this project demonstrates the practical application of core programming concepts, including file handling (for persistent data), control structures (loops/conditions), and modular design. It connects theoretical logic—represented through flowcharts and pseudocode—with a functional real-world implementation.
## Problem statement
Managing inventory manually is time-consuming and prone to human error. The specific challenge addressed in this project is to develop a software solution that automates the sales cycle for an electronics store.

The system addresses three core requirements:

Transparency: It must read and display real-time stock availability from a database file.

Billing Automation: It needs to generate accurate, timestamped invoices for every customer transaction.

Inventory Integrity: Crucially, the system must automatically update the stock levels in the source file immediately after a purchase to prevent selling out-of-stock items.
## Aim and objectives
Aim The primary aim of this project is to design and develop a functional "Electronic Store Management System" using the Python programming language. This project serves as a practical application of theoretical programming concepts to solve a real-world business problem.

Objectives To achieve this aim, the following specific objectives have been set:

Technical Implementation: To implement robust File I/O (Input/Output) operations, ensuring that inventory data is accurately read from and written back to text files.

Data Management: To utilize appropriate Python data structures (such as Lists and Dictionaries) to efficiently handle product details and customer transactions.

Code Logic: To apply control flow mechanisms (loops and conditional statements) to validate user inputs and calculate dynamic discounts.

Development Process: To conduct necessary research on retail billing standards and ensure the project is delivered within the stipulated timeframe.
## Purposed approach
...
## Target audience
Small Business Owners: The application is designed for small-scale retail owners (specifically in the electronics sector) who require a lightweight, offline solution to digitize their billing process and manage inventory without the complexity of enterprise software
